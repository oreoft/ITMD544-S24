...
export default {
  ...
  methods: {
    async fetchMovies() {
      const response = await axios.get(`${process.env.VUE_APP_HOST}/movies`);
      this.movies = response.data;
      this.fetchUsers(); // 每次获取电影列表后也更新用户列表
    },
    async handleDeleteMovie(movieId) {
      try {
        await axios.delete(`${process.env.VUE_APP_HOST}/movies/${movieId}`);
    ...
    async fetchUsers() {
      const response = await axios.get(`${process.env.VUE_APP_HOST}/users`);
